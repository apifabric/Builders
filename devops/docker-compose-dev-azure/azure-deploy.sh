...
resourcename="builders"  # lower case, only
resourcegroup="builders_rg"
...
# git clone https://github.com/apilogicserver/builders.git
# cd builders
...
az group create --name builders_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group builders_rg --sku S1 --is-linux
az container create --resource-group builders_rg --name builders --image apilogicserver/builders:latest --dns-name-label builders --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//builders.westus.azurecontainer.io:5656'
